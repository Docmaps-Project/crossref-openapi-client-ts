...
import { DepositMessage } from '../model/depositMessage';
import { DepositsMessage } from '../model/depositsMessage';
...
export class DepositsApi {
     ...
     */
    public async depositsGet (filter?: string, query?: string, rows?: number, mailto?: string, offset?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: DepositsMessage;  }> {
        const localVarPath = this.basePath + '/deposits';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: DepositsMessage;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "DepositsMessage");
     ...
     * @param id 
     */
    public async depositsIdGet (id: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: DepositMessage;  }> {
        const localVarPath = this.basePath + '/deposits/{id}'
        ...
        if (id === null || id === undefined) {
            throw new Error('Required parameter id was null or undefined when calling depositsIdGet.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: DepositMessage;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "DepositMessage");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
     ...
     */
    public async depositsPost (body: string, test?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: DepositMessage;  }> {
        const localVarPath = this.basePath + '/deposits';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        ...
        if (body === null || body === undefined) {
            throw new Error('Required parameter body was null or undefined when calling depositsPost.');
        }
