...
import { JournalMessage } from '../model/journalMessage';
import { JournalsMessage } from '../model/journalsMessage';
...
export class JournalsApi {
     ...
     */
    public async journalsGet (cursor?: string, query?: string, rows?: number, mailto?: string, offset?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: JournalsMessage;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: JournalsMessage;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "JournalsMessage");
     ...
     * @param issn The ISSN identifier associated with the journal
     */
    public async journalsIssnGet (issn: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: JournalMessage;  }> {
        const localVarPath = this.basePath + '/journals/{issn}'
            .replace('{' + 'issn' + '}', encodeURIComponent(String(issn)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        // give precedence to 'application/json'
        ...
        // verify required parameter 'issn' is not null or undefined
        if (issn === null || issn === undefined) {
            throw new Error('Required parameter issn was null or undefined when calling journalsIssnGet.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: JournalMessage;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "JournalMessage");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
     ...
     * @param cursor Exposes the ability to deep page through large result sets, where offset would cause performance problems
     */
    public async journalsIssnWorksGet (issn: string, rows?: number, order?: string, facet?: string, sample?: number, sort?: string, offset?: number, mailto?: string, select?: string, query?: string, filter?: string, cursor?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: WorksMessage;  }> {
        const localVarPath = this.basePath + '/journals/{issn}/works'
            .replace('{' + 'issn' + '}', encodeURIComponent(String(issn)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        ...
        // verify required parameter 'issn' is not null or undefined
        if (issn === null || issn === undefined) {
            throw new Error('Required parameter issn was null or undefined when calling journalsIssnWorksGet.');
        }
