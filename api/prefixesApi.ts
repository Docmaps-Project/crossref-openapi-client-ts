...
import { PrefixMessage } from '../model/prefixMessage';
...
export enum PrefixesApiApiKeys {
}
...
export class PrefixesApi {
    ...
    public setApiKey(key: PrefixesApiApiKeys, value: string) {
        (this.authentications as any)[PrefixesApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param prefix 
     */
    public async prefixesPrefixGet (prefix: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PrefixMessage;  }> {
        const localVarPath = this.basePath + '/prefixes/{prefix}'
        ...
        if (prefix === null || prefix === undefined) {
            throw new Error('Required parameter prefix was null or undefined when calling prefixesPrefixGet.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PrefixMessage;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "PrefixMessage");
     ...
     */
    public async prefixesPrefixWorksGet (prefix: string, rows?: number, order?: string, facet?: string, sample?: number, sort?: string, offset?: number, mailto?: string, select?: string, query?: string, filter?: string, cursor?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: WorksMessage;  }> {
        const localVarPath = this.basePath + '/prefixes/{prefix}/works'
            .replace('{' + 'prefix' + '}', encodeURIComponent(String(prefix)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        ...
        // verify required parameter 'prefix' is not null or undefined
        if (prefix === null || prefix === undefined) {
            throw new Error('Required parameter prefix was null or undefined when calling prefixesPrefixWorksGet.');
        }
