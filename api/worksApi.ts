...
import { AgencyMessage } from '../model/agencyMessage';
...
export class WorksApi {
     ...
     * @param doi The DOI identifier associated with the work
     */
    public async worksDoiAgencyGet (doi: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: AgencyMessage;  }> {
        const localVarPath = this.basePath + '/works/{doi}/agency'
        ...
        if (doi === null || doi === undefined) {
            throw new Error('Required parameter doi was null or undefined when calling worksDoiAgencyGet.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: AgencyMessage;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "AgencyMessage");
     ...
     * @param doi The DOI identifier associated with the work
     */
    public async worksDoiGet (doi: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: WorkMessage;  }> {
        const localVarPath = this.basePath + '/works/{doi}'
            .replace('{' + 'doi' + '}', encodeURIComponent(String(doi)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        ...
        // verify required parameter 'doi' is not null or undefined
        if (doi === null || doi === undefined) {
            throw new Error('Required parameter doi was null or undefined when calling worksDoiGet.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: WorkMessage;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "WorkMessage");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
     ...
     */
    public async worksGet (rows?: number, order?: string, facet?: string, sample?: number, sort?: string, offset?: number, mailto?: string, select?: string, query?: string, filter?: string, cursor?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: WorksMessage;  }> {
